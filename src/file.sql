...
(UniqueID serial primary key ,
 ...
 room int,
 furniture boolean,
 address_id int references address(UniqueID),
 owner_id int references owners(UniqueID));

drop table rent_info;
create table rent_info
(UniqueID serial primary key ,
 owner_id int references owners(UniqueID),
 customer_id int references customers(UniqueID),
 agency_id int references agencies(UniqueID),
 house_id int references houses(UniqueID));
...
alter table agencies  add constraint constraint_chek check   (phone_number LIKE '+996_________');
drop table agencies;
alter table rent_info add column marital_status marital_status  ;
...
select owners.first_name, houses from houses inner join owners on houses.owner_id = owners.UniqueID where owners.first_name like '______';
select houses.price from houses where price between '1500' and '2000';
select houses from houses where houses.UniqueID in (5,6,7,8,9,10);
select owners.first_name,customers.first_name